...
def _default_images_dir() -> Path | None:
    """Derive a default images directory from config.test.test_* if available."""
    test_cfg = fast_split.CONFIG.get("test", {})
    dataset_name = test_cfg.get("test_dataset", "")
    dataset_cfg = test_cfg.get(f"test_{dataset_name}", {})
    raw = dataset_cfg.get("test_img_path")
...
def batch_split(
    ...
    *,
    ...
    """
    使用 fast_split 对目录中的所有影像批量裁剪/推理并输出结果。
...
def parse_args() -> argparse.Namespace:
    default_dir = _default_images_dir()
    ...
    parser.add_argument(
        ...
        type=Path,
        default=default_dir,
        help="待处理影像所在目录（默认尝试读取 config.test 中的 test_img_path 所在目录）",
    )
    parser.add_argument(
        "--labels-dir",
        type=Path,
